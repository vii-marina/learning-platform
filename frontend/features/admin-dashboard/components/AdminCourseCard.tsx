...
type AdminCourseCardProps = {
  ...
  onUpdateStatus: (
    course: AdminDashboardCourseSummary,
    action: Exclude<AdminCourseCardAction, "delete" | "permanent-delete">
  ) => void;
};
...
export function AdminCourseCard({
  ...
  const isDeleted = isDeletedAdminCourse(course);
      ...
      <div className="flex flex-1 flex-col pt-3">
        ...
        {isDeleted && course.deleted_at ? (
          <div className="mt-3 rounded-xl border border-rose-100 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700">
            Видалено: {new Date(course.deleted_at).toLocaleString("uk-UA")}
          </div>
        ) : null}
    ...
    >
      ...
      <div className="mt-auto space-y-2 pt-4">
        {isDeleted ? (
          <Button
            type="button"
            disabled={isActionBusy || !onPermanentDelete}
            onClick={() => onPermanentDelete?.(course)}
            className="w-full border-rose-700 bg-rose-700 text-white hover:bg-rose-800"
          >
            {actionInFlight === "permanent-delete" ? (
